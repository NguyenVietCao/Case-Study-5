...
import {getFindById, editService, getListServiceName } from "../service/ServiceSever";

function EditService() {
  const param = useParams();
  const [service, setService] = useState();
  const [serviceNames, setserviceName] = useState([]);
  const getFindByIds = async (id) => {
    const sv = await getFindById(id);
    setService(sv);
  };
    ...
    <>
      <Formik
        initialValues={{
          ...
          usableArea: service?.usableArea,
          rentalFee: service?.rentalFee,
          quantity: service?.quantity,
          rentalType: service?.rentalType,
        }}
        validationSchema={Yup.object({
          serviceName: Yup.string().required("this is field must be fill"),
          usableArea: Yup.string().required("this is field must be fill"),
